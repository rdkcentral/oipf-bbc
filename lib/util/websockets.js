 ...
 *  205  getWebSocket    — Firebolt endpoint not defined on window.__firebolt.endpoint
 *
...
/**
 * Resolve the live Firebolt websocket URL from the global scope.
 *
 * The endpoint is published by the runtime on window.__firebolt.endpoint and
 * carries a per-app-instance session token, so it can't be hardcoded and must
 * be read at connection time. Treated as an opaque URL but validated to be a
 * non-empty ws:// or wss:// string.
 * @ignore
 * @return {String} the resolved websocket URL
 * @throws {OipfError} 205 if the endpoint is missing or not a ws(s):// URL
 */
function resolveFireboltEndpoint() {
    const endpoint = typeof window !== 'undefined' && window.__firebolt && window.__firebolt.endpoint;
    if (typeof endpoint !== 'string' || !/^wss?:\/\//.test(endpoint)) {
        const got = typeof endpoint === 'string' ? endpoint.replace(/\?.*$/, '?[redacted]') : String(endpoint);
        throw new OipfError(205, `Firebolt endpoint is not defined on window.__firebolt.endpoint (got ${got})`);
    }
    return endpoint;
}
...
function getWebSocket(target) {
    function createWebSocket() {
        return new Promise((resolve, reject) => {
            let url;
            try {
                url = target === TARGET_URLS.Firebolt ? resolveFireboltEndpoint() : target;
            } catch (err) {
                // Defer deletion so it runs after getWebSocket assigns websocketPool[target].
                Promise.resolve().then(() => { delete websocketPool[target]; });
                reject(err);
                return;
            }
