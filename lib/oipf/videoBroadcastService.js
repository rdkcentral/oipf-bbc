 ...
 * Resolves with the host's currently-tuned channel (or null if it isn't in our
 * channel list) as soon as it's resolved — for the STOPPED branch this is before
 * the tune itself completes; that outcome still only surfaces via setChannel's
 * ChannelChangeSucceeded/ChannelChangeError events.
 */
export function bindToCurrentChannel() {
    return init().then(function() {
        return resolveCurrentChannel()
    ...
    listenForTuneCompletion(chanId)
        ...
        .catch(function(error) {
            ...
            if (error instanceof OipfError) {
                dispatchError(channel, oipfErrorCode[error.type] || 100);
            } else {
                dispatchError(channel, 100);
            }
        });
